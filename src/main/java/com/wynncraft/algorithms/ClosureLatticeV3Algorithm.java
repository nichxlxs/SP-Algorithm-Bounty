...
public class ClosureLatticeV3Algorithm implements IAlgorithm<WynnPlayer> {
    ...
    public Result run(WynnPlayer player) {
        ...
        // Fixpoint: earlier-deferred non-negative items may now pass minSP.
        boolean extractedMore = extractedCount > 0;
        while (extractedMore) {
            extractedMore = false;
            for (int k = 0; k < wl; k++) {
                int i = wlIdx[k];
                ...
                int[] r = wlReq[k];
                if ((r[0] > 0 && ext0 + neg0 < r[0]) || (r[1] > 0 && ext1 + neg1 < r[1])
                    || (r[2] > 0 && ext2 + neg2 < r[2]) || (r[3] > 0 && ext3 + neg3 < r[3])
                    || (r[4] > 0 && ext4 + neg4 < r[4])) {
                    continue;
                }
        ...
        for (int k = 0; k < wl; k++) {
            ...
            if (!negFlag[i] && (reqLanes & riskyMask) == 0) {
                int f = forcedCount++;
                forcedItem[f] = i;
                fReq[f] = r;
                fBon[f] = b;
                ...
                bKeys[p] = negFlag[i] ? (1 << 30) - rpbSum : r0 + r1 + r2 + r3 + r4;
            }
        }
